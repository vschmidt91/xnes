...
typed dataclass instances or nested plain dicts. For overlapping or
out-of-order evaluations, use `ask_trial()` / `tell_trial()`. For deterministic
...
Context matching uses explicit string labels, while explicit trial handles
remain runtime-only and are not persisted. Mean snapshots from `ask_best()`
are returned directly as the schema type.
